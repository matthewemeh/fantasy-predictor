...
const MoreScreen = ({ visible }) => {
    ...
    const aboutMessage = 'This app will help you in planning your team properly for gameweeks in the Fantasy Premier League. By following our regularly predicted scores and scout selections, you will arrive at the perfect XI for your team.';
        ...
        if (update && appVersion) {
            if (sum(numbersInString(update?.currentVersion)) > sum(numbersInString(appVersion))) {
                if (update?.updateLink)
                    Linking.openURL(update.updateLink);
                else
                    ToastAndroid.show('An error has occured', 2000);
            }
            else
                ToastAndroid.show('App is up to date', 1000);
        }
        else
            ToastAndroid.show('An error has occured', 2000);
    };
    const rate = () => {
        if (update?.rateUsUrl)
            Linking.openURL(update.rateUsUrl);
        else
            ToastAndroid.show('An error has occured', 2000);
    };
    ...
    const buttons = [
        ...
        },
        {
            ...
            expandable: true,
            expandedHeight: 170,
            expandedContent: (<View style={{ ...styles.contentView, paddingHorizontal: '3%' }}>
        ...
        },
        {
            ...
            expandable: true,
            expandedHeight: 240,
            expandedContent: (<View style={{ ...styles.contentView, marginTop: 5 }}>
    ...
    return (<ScrollView style={{ ...styles.mainView, display: visible ? 'flex' : 'none' }}>
      {buttons.map(({ expandable, expandedContent, iconName, title, onPress, expandedHeight }) => (<MoreBubble key={title} title={title} onPress={onPress} iconName={iconName} expandable={expandable} expandedHeight={expandedHeight} expandedContent={expandedContent}/>))}

      <AdMobBanner style={styles.bottomView} servePersonalizedAds={true} bannerSize='smartBannerLandscape' onDidFailToReceiveAdWithError={() => { }} adUnitID='ca-app-pub-7152054343360573/9830430705'/>
