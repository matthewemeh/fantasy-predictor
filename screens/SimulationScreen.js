...
import { View, Text, Modal, StyleSheet, ToastAndroid, ImageBackground, TouchableOpacity, } from 'react-native';
...
import { player, colors, findData, findFontSize, unknownImage, DEVICE_HEIGHT, randomSelect, fieldConstants, findPlayerInfo, DEFAULT_AD_HEIGHT, descendingPointsOrder, findOpponentAbbreviation, } from '../utilities';
import { AppContext } from '../App';
const SimulationScreen = ({ visible }) => {
    const initializeId = async () => await setTestDeviceIDAsync('EMULATOR');
    const { playerKit, goalieKit, teamsData, playerData, fieldImage, nextOpponent, positionData, setAlertVisible, setAlertComponents, } = useContext(AppContext);
    // componentDidMount
    useEffect(() => {
        initializeId();
    }, []);
    ...
    useEffect(() => setSaveButtonEnabled(playerInfo.every(({ playerName }) => playerName)), [playerModalVisible1, saveButtonEnabled, playerInfo]);
    ...
    const teamEnough = (teamName) => {
        const goalkeepersID = positionData?.find(({ singular_name }) => singular_name.toLowerCase() === 'goalkeeper')?.id;
        ...
        if ((playerData?.filter(({ team, element_type }) => team === teamID && element_type === goalkeepersID).length || 0) < 1) {
            return false;
        }
        else if ((playerData?.filter(({ team, element_type }) => team === teamID && element_type === defendersID).length || 0) < 4) {
            return false;
        }
        else if ((playerData?.filter(({ team, element_type }) => team === teamID && element_type === midfieldersID).length || 0) < 5) {
            return false;
        }
        else if ((playerData?.filter(({ team, element_type }) => team === teamID && element_type === forwardsID).length || 0) < 1) {
            return false;
        }
        return true;
    };
    ...
    const findBestFormation = (teamName) => {
        const forwardsID = positionData?.find(({ singular_name }) => singular_name.toLowerCase() === 'forward')?.id;
        const teamID = teamsData?.find(({ name }) => name === teamName)?.id;
        return (playerData?.filter(({ team, element_type }) => team === teamID && element_type === forwardsID).length || 0) >= 2
            ? '4-4-2'
            : '4-5-1';
    };
    const getTeam = (teamName) => {
        ...
        const bestMidfielders = (midfielders && midfielders.slice(0, parseInt(formation.charAt(2)))) || [];
    ...
    return (<View style={{ ...styles.main, display: visible ? 'flex' : 'none' }}>
      <Modal visible={playerSelectVisible} onRequestClose={onSelectModalClose} transparent={true}>
        <View style={styles.mainSelect}>
          <PlayerSelectModal playerInfo={playerInfo} chosenTeam={chosenTeam} posPickerEnabled={false} teamPickerEnabled={false} currentIndex={currentIndex} playerKit={playerKit || {}} goalieKit={goalieKit || {}} playerData={playerData || []} visible={playerModalVisible1} setChosenTeam={setChosenTeam} chosenPosition={chosenPosition} nextOpponent={nextOpponent || []} setChosenPosition={setChosenPosition} key={chosenPosition.concat(chosenTeam)} setPlayerModalVisible={setPlayerModalVisible1}/>

          <View style={styles.headerSelect}>
            <PickerBox enabled={true} list={formations} selectedValue={chosenFormation} selectedItemHandler={setChosenFormation} extraStyles={{ width: '30%', height: '50%' }}/>
          </View>

          <View style={styles.bodySelect}>
            <ImageBackground imageStyle={styles.imageBackground} source={typeof fieldImage == 'string'
            ? { uri: fieldImage, headers: { Accept: '*/*' } }
            : fieldImage} style={styles.imgViewStyle}>
            ...
            <FilterBar type={type} pickerEnabled={false} chosenTeam={chosenTeam} setChosenTeam={setChosenTeam} teams={['All Teams', ...teams]} confirmRemoveAll={confirmRemoveAll} randomizePlayers={randomizePlayers}/>
          </View>

          <View style={styles.admobContainer}>
            {0.23 * 0.5 * DEVICE_HEIGHT >= DEFAULT_AD_HEIGHT ? (<AdMobBanner style={styles.admobView} servePersonalizedAds={true} bannerSize='smartBannerLandscape' onDidFailToReceiveAdWithError={() => { }} adUnitID='ca-app-pub-7152054343360573/9830430705'/>) : (<View style={styles.admobView}/>)}
            <View style={styles.bottomView}>
              <Button command={save} buttonText='Save' enabled={saveButtonEnabled} buttonColor={colors.forward} buttonTextColor={colors.white} extraStyles={{ width: '90%', height: '60%' }}/>
            </View>
          </View>
        </View>
      </Modal>

      <PlayerSelectModal chosenTeam={chosenTeam} posPickerEnabled={false} teamPickerEnabled={false} playerKit={playerKit || {}} goalieKit={goalieKit || {}} currentIndex={currentIndex} visible={playerModalVisible2} playerData={playerData || []} setChosenTeam={setChosenTeam} chosenPosition={chosenPosition} nextOpponent={nextOpponent || []} setChosenPosition={setChosenPosition} key={chosenPosition.concat(chosenTeam)} setPlayerModalVisible={setPlayerModalVisible2} squadInfo={chosenTeam === chosenTeam1 ? squad1 : squad2}/>

      <SimulationBoard squad1={squad1} squad2={squad2} visible={boardVisible} teamName1={chosenTeam1} teamName2={chosenTeam2} teamsData={teamsData || []} positionData={positionData || []} setBoardVisible={setBoardVisible} shortTeamName1={teamsData?.find(({ name }) => name === chosenTeam1)?.short_name || ''} shortTeamName2={teamsData?.find(({ name }) => name === chosenTeam2)?.short_name || ''}/>

      <View style={styles.header}>
        <PickerBox enabled={true} selectedValue={chosenTeam1} list={['All Teams', ...teams]} extraStyles={{ width: '45%' }} selectedItemHandler={setChosenTeam1} extraListStyles={{ top: '1%', left: '3%', width: '42.5%', marginBottom: '35%' }}/>

        <Text allowFontScaling={false} style={styles.textStyle}>
          Vs
        </Text>

        <PickerBox enabled={true} selectedValue={chosenTeam2} list={['All Teams', ...teams]} extraStyles={{ width: '45%' }} selectedItemHandler={setChosenTeam2} extraListStyles={{ top: '1%', left: '54.6%', width: '42.5%', marginBottom: '35%' }}/>
      </View>

      <View style={styles.body}>
        <View style={styles.chooseSquadButton}>
          <Button command={chooseSquad} buttonText='Choose Squads' buttonColor={colors.stratos} buttonTextColor={colors.white} extraStyles={{ width: '95%' }} enabled={simulateButtonEnabled} extraTextStyles={{ fontSize: findFontSize(15) }}/>
        </View>
        ...
        <View style={styles.squadView}>
          <View style={{ ...styles.listStyle, paddingRight: '0.5%' }}>
            ...
            if (player) {
                ...
                return (<TouchableOpacity key={index} activeOpacity={0.6} onPress={() => playerCommand(chosenTeam1, id, index)} style={{
                        ...styles.touchView,
            ...
            return (<TouchableOpacity key={index} onPress={() => { }} activeOpacity={0.6} style={{
                    ...styles.touchView,
                    borderTopRightRadius: 0,
                    borderBottomRightRadius: 0,
                }}>
          ...
          <View style={{ ...styles.listStyle, paddingLeft: '0.5%' }}>
            ...
            if (player) {
                const { id, web_name } = player;
                return (<TouchableOpacity key={index} activeOpacity={0.6} onPress={() => playerCommand(chosenTeam2, id, index)} style={{
                        ...styles.touchView,
      ...
      <View style={styles.footer}>
        <Button command={openBoard} buttonText='SIMULATE' buttonColor={colors.stratos} buttonTextColor={colors.white} extraStyles={{ width: '40%' }} enabled={simulateButtonEnabled} extraTextStyles={{ fontSize: findFontSize(17) }}/>
      </View>
    </View>);
};
