...
const LogDisplay = ({ info, version }) => (<View style={{ ...styles.mainView, height: DEVICE_HEIGHT * 0.05 * (info.length + 1) }}>
    <Text allowFontScaling={false} style={styles.versionText}>
      {version}
    </Text>

    {info.map(([logType, logMessage]) => (<View style={styles.logView} key={logMessage}>
        <Text allowFontScaling={false} style={{ ...styles.signView, color: logType === 'add' ? colors.emerald : colors.red }}>
          {logType === 'add' ? '+' : '-'}
        </Text>
