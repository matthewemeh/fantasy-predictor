...
const PickerBox = ({ list, enabled, extraStyles, selectedValue, extraListStyles, extraTextItemStyles, selectedItemHandler, extraSelectedValueStyles, }) => {
    ...
    return (<TouchableOpacity onPress={togglePicker} activeOpacity={enabled ? 0.9 : 1} style={{ ...styles.mainView, ...extraStyles }}>
      <AnimatableText duration={400} numberOfLines={1} allowFontScaling={false} animation={enabled ? {} : animations.pickerDisabled} style={{
      ...
      <Modal visible={opened && enabled} onRequestClose={closePicker} transparent={true}>
        <TouchableOpacity activeOpacity={1} onPress={closePicker} style={styles.modalView}>
          <View style={{ ...styles.listView, ...extraListStyles }}>
            <ScrollView>
              {list.map(item => (<TouchableOpacity key={item} activeOpacity={item === selectedValue ? 1 : 0.8} onPress={item === selectedValue ? () => { } : () => onChange(item)} style={{
                ...styles.listItem,
                backgroundColor: item === selectedValue ? colors.alto : colors.white,
            }}>
                  <Text numberOfLines={1} allowFontScaling={false} style={{ ...styles.textItem, ...extraTextItemStyles }}>
                    {item}
                  </Text>
                </TouchableOpacity>))}
            </ScrollView>
