...
import { colors, shuffle, findFontSize, randomSelect, DEVICE_HEIGHT, getRndInteger, } from '../utilities';
const SimulationBoard = ({ squad1, squad2, visible, teamsData, teamName1, teamName2, positionData, shortTeamName1, shortTeamName2, setBoardVisible, }) => {
    const MIN_LENGTH = 15;
    ...
    const updateTime = () => {
        ...
        if (visible) {
            if (timeInSeconds + increment < 5400)
                setTimeInSeconds(timeInSeconds + increment);
            else
                setTimeInSeconds(5400);
        }
    };
    const initDataArrays = () => {
        ...
        squad1.forEach(player => {
            if (player) {
                ...
                const defenderGoalsAccepted = (element_type === defendersID && goals_scored > 3) || element_type !== defendersID;
        ...
        while (savers1.length < dataLength2)
            savers1.push('');
        while (goalScorers1.length < dataLength1)
            goalScorers1.push('');
        while (assistProviders1.length < dataLength1)
            assistProviders1.push('');
        shuffle(savers1);
        ...
        // fill up remaining data slots until length equals required lengths for each team
        while (savers2.length < dataLength1)
            savers2.push('');
        while (goalScorers2.length < dataLength2)
            goalScorers2.push('');
        while (assistProviders2.length < dataLength2)
            assistProviders2.push('');
        shuffle(savers2);
    ...
    return (<Modal visible={visible} onRequestClose={onRequestClose}>
      <View style={styles.main}>
        <View style={styles.header}>
          ...
          <ScoreBoard score1={score1} score2={score2} shortTeamName1={shortTeamName1} shortTeamName2={shortTeamName2}/>
        </View>

        <LinearGradient colors={[colors.athens, colors.alto]} style={styles.gradientView}/>

        <FlatList data={events} style={styles.body} renderItem={renderItem} getItemLayout={getItemLayout} ItemSeparatorComponent={separator}/>

        <LinearGradient colors={[colors.alto, colors.athens]} style={styles.gradientView}/>

        <View style={styles.bottom}>
          <Icon name='undo' onPress={refresh} type='font-awesome' color={colors.forward} size={findFontSize(45)}/>
          <Icon name='times' type='font-awesome' color={colors.forward} size={findFontSize(45)} onPress={onRequestClose}/>
        </View>
      </View>
    </Modal>);
};
