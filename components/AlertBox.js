...
const AlertBox = ({ title, message, buttons, visible, onRequestClose }) => (<Modal visible={visible} transparent={true} onRequestClose={onRequestClose}>
    <View style={styles.mainView} onTouchStart={onRequestClose}>
      <View style={styles.containerView} onTouchStart={e => e.stopPropagation()}>
        ...
        <View style={styles.buttonsView}>
          {buttons.map(({ text, onPress }) => (<Button key={text} enabled={true} command={onPress} buttonText={text} activeOpacity={1} buttonColor={colors.white} buttonTextColor={colors.stratos} extraTextStyles={{ fontSize: findFontSize(11) }} extraStyles={{ width: '19%', height: '80%', marginRight: '6%' }}/>))}
        </View>
      </View>
    </View>
  </Modal>);
