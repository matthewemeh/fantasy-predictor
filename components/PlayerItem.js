...
import { colors, DEVICE_WIDTH, DEVICE_HEIGHT, findRotationAngle, findScaledFontSize, } from '../utilities';
...
const PlayerItem = ({ command, enabled, teamName, shirtImage, playerName, playerIndex, extraStyles, activeOpacity, }) => {
    const rotationAngle = findRotationAngle(playerIndex);
    const arrowImage = require('../assets/arrowUp.webp');
    return (<TouchableOpacity onPress={enabled ? command : () => { }} activeOpacity={enabled ? activeOpacity : 1} style={{
            ...styles.mainView,
            ...extraStyles,
            backgroundColor: enabled ? colors.white : colors.alto,
        }}>
      <View style={styles.imageView}>
        <Image source={typeof shirtImage == 'string'
            ? { uri: shirtImage, headers: { Accept: '*/*' } }
            : shirtImage} style={styles.imageStyle}/>
      </View>

      <View style={styles.arrowView}>
        <Image source={arrowImage} style={{ ...styles.arrowStyles, transform: [{ rotate: rotationAngle }] }}/>
      </View>
