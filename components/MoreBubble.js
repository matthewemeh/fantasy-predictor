...
const MoreBubble = ({ onPress, title, iconName, expandable, expandedContent, expandedHeight, }) => {
    ...
    return (<AnimatableView duration={400} style={styles.viewStyle} onTouchStart={handlePress} animation={expanded ? animations.collapse : animations.expand}>
      <View style={styles.touchableHeaderView}>
        <Icon name={iconName} type='font-awesome' style={styles.iconView} size={findFontSize(25)} color={colors.blueRibbon}/>

        <Text allowFontScaling={false} style={styles.textStyle}>
          {title}
        </Text>

        {expandable && (<AnimatableView duration={400} animation={expanded ? animations.rotate0 : animations.rotate180}>
            <Icon name='caret-down' type='font-awesome' size={findFontSize(25)} color={colors.blueRibbon} style={styles.dropdownIcon}/>
          </AnimatableView>)}
      </View>
