...
import { colors, findFontSize, DEVICE_WIDTH, DEVICE_HEIGHT, findScaledFontSize, findPlayingChanceColor, } from '../utilities';
...
const PlayerView = ({ imgVal, command, isCaptain, playerName, longCommand, extraStyles, playerContent, activeOpacity, extraTextStyles1, extraTextStyles2, extraShirtStyles, chanceOfPlayingNextRound, }) => {
    const animations = {
        captain: { from: { opacity: 0 }, to: { opacity: 1 } },
        unCaptain: { from: { opacity: 0 }, to: { opacity: 0 } },
    };
    return (<View style={{ ...styles.mainView, ...extraStyles }}>
      <AnimatableText duration={400} allowFontScaling={false} style={styles.captainBadgeText} animation={isCaptain && playerName ? animations.captain : animations.unCaptain}>
        C
      </AnimatableText>

      <TouchableOpacity onPress={command} style={styles.shirtView} onLongPress={longCommand} activeOpacity={activeOpacity || 0.6}>
        <Image style={{ ...styles.image, ...extraShirtStyles }} source={typeof imgVal == 'string' ? { uri: imgVal, headers: { Accept: '*/*' } } : imgVal}/>
      </TouchableOpacity>

      <Text numberOfLines={1} allowFontScaling={false} style={{ ...styles.nameView, ...extraTextStyles1 }}>
        {playerName}
      </Text>
