...
const LogNote = ({ visible, onRequestClose }) => (<Modal visible={visible} transparent={true} animationType='slide' onRequestClose={onRequestClose}>
    <View style={styles.containerView} onTouchStart={onRequestClose}>
      <View style={styles.mainView} onTouchStart={e => e.stopPropagation()}>
        ...
        <FlatList data={logs.logs} style={styles.bodyView} keyExtractor={({ version }) => version} renderItem={({ item }) => <LogDisplay info={item.info} version={item.version}/>}/>
      </View>
    </View>
  </Modal>);
